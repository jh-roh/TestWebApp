...
exports.XRCellsurface = exports.XRCrossTabCellViewModel = exports.kindToString = void 0;
...
var XRCrossTabCellViewModel = (function (_super) {
    ...
    function XRCrossTabCellViewModel(model, parent, serializer) {
        ...
        _this._disposables.push(_this.kind.subscribe(function (kind) {
            if (_this._textFormatString() === '' && _this.field() && (kind == enums_1.CellKind.RowTotalHeader
                || kind == enums_1.CellKind.ColumnTotalHeader))
                _this.textFormatString('Total {0}');
        }));
        _this._disposables.push(ko.computed(function () {
            if (_this._text() === '' && _this.isIndependant() && _this.kind() != enums_1.CellKind.Corner) {
            ...
            if ((_this.kind() == enums_1.CellKind.Corner || _this.kind() == enums_1.CellKind.DataHeader) && _this.fieldName() && _this.fieldName() != _this._oldFieldName) {
                (_this._oldFieldName == _this.text() || _this.text() === '') && _this.text(_this.fieldName());
        ...
        _this._disposables.push(_this.rowVisible = ko.pureComputed({
            ...
            write: function (value) {
                for (var i = 0; i < _this._rowSpan(); i++) {
                    parent._rowDefinitions()[_this._rowIndex() + i].visible(value);
                }
            }
        }));
        _this._disposables.push(_this.columnVisible = ko.pureComputed({
            ...
            write: function (value) {
                for (var i = 0; i < _this._columnSpan(); i++) {
                    parent._columnDefinitions()[_this._columnIndex() + i].visible(value);
                }
            }
        }));
    ...
    XRCrossTabCellViewModel.prototype._createParametersExpressionCategory = function () {
        var crossTabParameters = this.parent.controlParameters();
        crossTabParameters.forEach(function (x) { return x.initDataMemberInfo(); });
        return analytics_widgets_internal_1.createExpressionEditorCollectionToolOptions(crossTabParameters.map(function (x) {
            return {
                text: x.name,
                val: '?' + x.name,
    ...
    XRCrossTabCellViewModel.prototype.isPropertyVisible = function (name, isPopularProperty) {
        if (isPopularProperty === void 0) { isPopularProperty = false; }
    ...
    XRCrossTabCellViewModel.prototype.customizeExpressionCategories = function (tools, categories) {
        var crossTabParameters = this.parent.controlParameters();
        var crossTabFields = this.parent.getFields();
        ...
        if (fieldsCategory) {
            ...
            var fields = fieldsCategory.content.data.fields();
            if (fields) {
                fields.treeListController.customFilter = function (path) {
                    return crossTabFields.some(function (x) { return analytics_internal_1.getFullPath(x.getPath(''), x.fieldName()) === path; });
                };
            }
            else {
