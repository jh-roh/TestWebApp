...
exports.ConfirmOnLosingChangesSettings = exports.Settings = void 0;
...
var search_1 = require("../core/model/options/search");
...
var Settings = (function () {
    function Settings() {
        ...
        this.search = new search_1.SearchSettings();
    ...
    Settings.parse = function (settings, subDocumentCreator) {
    ...
    Settings.parseFieldsSettings = function (settings, result) {
        if (common_1.isDefined(settings.fields)) {
            ...
            if (common_1.isDefined(settings.fields.keepHyperlinkResultForInvalidReference))
    ...
    Settings.parsePdfSettings = function (settings, result) {
        if (common_1.isDefined(settings.pdf)) {
            ...
            if (settings.pdf.convertImageToCompatibleFormat)
                result.pdf.convertImageToCompatibleFormat = utils_1.convertToFunction(settings.pdf.convertImageToCompatibleFormat);
        }
    };
    Settings.parseSearchSettings = function (settings, result, subDocumentCreator) {
        if (common_1.isDefined(settings.search)) {
            if (settings.search.filterInterval) {
                var func_1 = utils_1.convertToFunction(settings.search.filterInterval);
                result.search.filterInterval =
                    function (subDocumentId, interval) {
                        return func_1(subDocumentCreator(subDocumentId), new interval_1.IntervalApi(interval.start, interval.length));
                    };
            }
        }
    };
