...
var ModelComparer = (function () {
    ...
    ModelComparer.prototype.compareSubDocumentsCore = function (source, target) {
        ...
        if (source.chunks.length != target.chunks.length)
            return check_all_1.ModelCheckerResult.SubDocument;
        var result = this.compareRangePermissions(source.rangePermissions, target.rangePermissions);
        if (result != check_all_1.ModelCheckerResult.None)
            return result;
    ...
    ModelComparer.prototype.compareRangePermissions = function (source, target) {
        if (source.length != target.length)
            return check_all_1.ModelCheckerResult.RangePermission;
        return list_1.ListUtils.anyOf(source, function (sourcePermission, index) { return !sourcePermission.equals(target[index]); }) ? check_all_1.ModelCheckerResult.RangePermission : check_all_1.ModelCheckerResult.None;
    };
