...
exports.RangeManipulator = void 0;
...
var RangeManipulator = (function (_super) {
    ...
    RangeManipulator.prototype.removeInterval = function (subDocInterval, removeTableIfItMatchesWithInterval, clearLastParagraphIfIntervalMatchesWholeDocument, forbidNonEmptyParagraphRemovingBeforeTable) {
        ...
        var intervalEndPosition = subDocInterval.interval.end;
        if (intervalEndPosition == documentEndPosition)
            subDocInterval.interval.length -= 1;
        else {
            var endParIndex = subDocInterval.subDocument.getParagraphIndexByPosition(intervalEndPosition);
            if (endParIndex > 0) {
                var endPar = subDocInterval.subDocument.paragraphs[endParIndex];
                var beforeEndPar = subDocInterval.subDocument.paragraphs[endParIndex - 1];
                var intervalEndsInParagraphStart = intervalEndPosition == endPar.startLogPosition.value;
                var isOnlyParagraphMarkRemoved = intervalEndsInParagraphStart && subDocInterval.interval.length == 1;
                var isParagraphEndsBeforeTable = intervalEndsInParagraphStart && !beforeEndPar.getTableCell() && endPar.getTableCell();
