...
var PictureLoader = (function () {
    ...
    PictureLoader.prototype.ensureAllPicturesPdfCompatible = function (timeout, convertImageToCompatibleFormat, callback) {
        var _this = this;
        this.ensureAllPicturesLoaded(timeout, function (_isLoaded) {
            if (common_1.isDefined(convertImageToCompatibleFormat)) {
                var infos = _this.modelManipulator.model.getAllImages();
                var promises_1 = [];
                number_1.NumberMapUtils.forEach(infos, function (info) {
                    if (info.shouldMakeImagePdfCompatible()) {
                        var promise = convertImageToCompatibleFormat(info.base64);
                ...
                Promise.all(promises_1).then(function () { return callback(); });
            }
            else
                callback();
        });
