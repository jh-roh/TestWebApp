...
exports.RangedArg = exports.TocElement = exports.TocParserParameters = exports.FieldCodeParserToc = void 0;
...
var FieldCodeParserToc = (function (_super) {
    ...
    FieldCodeParserToc.prototype.createTocElementsByOutlineLevel = function (params) {
        ...
        for (var i = 0, paragraph = void 0; paragraph = this.subDocument.paragraphs[i]; i++) {
            var styleOutlineLevel = paragraph.paragraphStyle.getMergedParagraphProperties().outlineLevel;
            var level = params.fromOutlineLevel && styleOutlineLevel <= 0 ?
            ...
            if (level > 0 && (!params.useSpecifiedLevels || (level >= params.specifiedLevelStart && level <= params.specifiedLevelEnd))) {
                var paragraphInterval = this.getParagraphInterval(paragraph);
                var bookmarkName = this.getBookmarkName(paragraphInterval);
    ...
    FieldCodeParserToc.getParagraphInterval = function (subDocument, paragraph) {
        var characterIterator = new model_iterator_1.ModelIterator(subDocument, false);
        var paragraphMarkPosition = paragraph.interval.end - 1;
        ...
        var intervalStart = characterIterator.getAbsolutePosition();
        var length = paragraph.interval.length - (intervalStart - paragraph.startLogPosition.value) - 1;
        return new fixed_1.FixedInterval(intervalStart, length);
    };
    FieldCodeParserToc.prototype.getParagraphInterval = function (paragraph) {
        return FieldCodeParserToc.getParagraphInterval(this.subDocument, paragraph);
    };
    FieldCodeParserToc.isWhiteSpace = function (ch) {
        return rich_utils_1.RichUtils.isWhitespace.test(ch) || ch == rich_utils_1.RichUtils.specialCharacters.NonBreakingSpace || ch == rich_utils_1.RichUtils.specialCharacters.PageBreak || ch == rich_utils_1.RichUtils.specialCharacters.ColumnBreak || ch == rich_utils_1.RichUtils.specialCharacters.LineBreak;
    };
