...
exports.FieldCodeParser = exports.FieldCodeParserHelper = exports.FieldParameter = exports.FieldSwitch = exports.FieldMailMergeType = exports.FieldSwitchType = exports.FieldCodeParserState = void 0;
...
var FieldCodeParser = (function () {
    ...
    FieldCodeParser.prototype.getFormattedResult = function (value) {
        ...
        if (type == FieldSwitchType.General && !string_1.StringUtils.isNullOrEmpty(arg)) {
    ...
    FieldCodeParser.prototype.capitalizesFirstLetterOfFirstWord = function (value) {
        var result = value;
        if (result.length > 0)
            result = this.replaceCharacter(result, 0, result[0].toLocaleUpperCase());
        return result;
    };
    FieldCodeParser.prototype.capitalizesFirstLetterOfEachWord = function (value) {
        var _this = this;
        var result = value.toLocaleLowerCase();
        var index = this.skipCharacters(0, result, function (str) { return !_this.isLetterOrDigit(str); });
        while (index < result.length) {
            result = this.replaceCharacter(result, index, result[index].toLocaleUpperCase());
            index = this.skipCharacters(index, result, function (str) { return _this.isLetterOrDigit(str); });
            index = this.skipCharacters(index, result, function (str) { return !_this.isLetterOrDigit(str); });
        }
        return result;
    };
    FieldCodeParser.prototype.replaceCharacter = function (source, index, chForReplace) {
        return source.substr(0, index) + chForReplace + source.substr(index + 1);
    };
    FieldCodeParser.prototype.skipCharacters = function (index, str, predicate) {
        while (index < str.length && predicate(str[index]))
            index++;
        return index;
    };
    FieldCodeParser.prototype.isLetterOrDigit = function (str) {
        if (str.length != 1)
            return false;
        if (str.match(/[0-9]/i))
            return true;
