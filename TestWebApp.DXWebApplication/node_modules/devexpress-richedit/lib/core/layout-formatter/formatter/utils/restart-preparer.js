...
exports.RestartPreparer = void 0;
...
var RestartPreparer = (function () {
    ...
    RestartPreparer.prototype.restartFromPage = function (pageIndex, isRestartNow, resetTableFormatter) {
        var restartPosition = this.calcRestartPosition(pageIndex);
    ...
    RestartPreparer.prototype.calcRestartPosition = function (pageIndex) {
        if (pageIndex == 0)
            return 0;
        var prevPage = this.manager.layout.pages[pageIndex - 1];
        var pageArea = list_1.ListUtils.last(prevPage.mainSubDocumentPageAreas);
        var column = pageArea.getLastColumn();
        var row = column.getLastRow();
        var box = list_1.ListUtils.last(row.boxes);
        var lastPosition = prevPage.getPosition() + pageArea.pageOffset + column.pageAreaOffset + row.columnOffset + box.rowOffset + box.getLength();
        var tableCellInfo = row.tableCellInfo;
        if (!tableCellInfo || tableCellInfo.boundFlags.get(layout_table_cell_info_1.TableCellBoundFlags.StartOnThisColumn))
            return lastPosition;
