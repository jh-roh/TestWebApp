...
exports.RowSizesManager = void 0;
...
var RowSizesManager = (function () {
    ...
    RowSizesManager.prototype.tryPlaceNumberingBoxes = function () {
        var numberingListBox = this.row.numberingListBox;
        var textBox = numberingListBox.textBox;
        var offset = this.calculateNumberingListBoxOffset(numberingListBox.alignment, textBox.width);
        var startPos = this.rowFormattingInfo.currInterval.startOfFreeSpace - offset;
        textBox.x = startPos;
        var totalWidth = textBox.width;
        var separatorWidthDependsOnIntervalIndex = false;
        var separatorBox = numberingListBox.separatorBox;
        if (separatorBox) {
            var textBoxEndPos = startPos + this.row.numberingListBox.textBox.width;
            if (separatorBox.getType() == layout_box_1.LayoutBoxType.TabSpace) {
                var tabEndPosition = this.getTabEndPos(this.rowFormattingInfo.currIndex == 0, textBoxEndPos);
        ...
        if (this.rowFormattingInfo.isFloatingIntersectRow) {
            var indexOfFreeInterval = this.rowFormattingInfo.indexOfFreeInterval(totalWidth);
            if (indexOfFreeInterval < 0) {
                this.rowFormattingInfo.findNextYPos();
                this.rowFormattingInfo.currIndex = 0;
                this.restartAllRow();
    ...
    RowSizesManager.prototype.calculateNumberingListBoxOffset = function (alignment, boxWidth) {
        var offset = 0;
        if (alignment == list_level_properties_1.ListNumberAlignment.Center)
            offset = boxWidth / 2;
