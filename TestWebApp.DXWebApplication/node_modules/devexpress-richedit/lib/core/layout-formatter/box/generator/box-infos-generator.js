...
exports.BoxGenerator = void 0;
var enums_1 = require("../../../model/character/enums");
var character_properties_merger_1 = require("../../../model/properties-merger/character-properties-merger");
var character_style_1 = require("../../../model/character/character-style");
...
var color_model_info_1 = require("../../../model/color/color-model-info");
...
var BoxGenerator = (function () {
    ...
    BoxGenerator.prototype.addToBoxFieldInfo = function (wrap) {
        ...
        if (wrap.box.getType() == layout_box_1.LayoutBoxType.LayoutDependent)
            wrap.box.setType(BoxGenerator.MapFieldCodeToLayoutDependentBoxType[this.getFieldType(list_1.ListUtils.last(fieldsInfo))]);
    };
    BoxGenerator.prototype.getFieldType = function (fieldInfo) {
        if (!fieldInfo.fieldType)
            fieldInfo.fieldType = string_1.StringUtils.trim(this.subDocument.getText(fieldInfo.field.getCodeInterval()).split("\\")[0]).toUpperCase();
    ...
    BoxGenerator.prototype.makeParagraphAndSectionBox = function () {
        ...
        this.addNewBoxInfo(this.run.getType() == run_type_1.RunType.ParagraphRun ?
            new layout_paragraph_mark_box_1.LayoutParagraphMarkBox(charProps, charProps.getLayoutColorInfo(this.colorProvider), this.isLastParagraphInCell()) :
            new layout_section_mark_box_1.LayoutSectionMarkBox(charProps, charProps.getLayoutColorInfo(this.colorProvider)));
        this.offsetStartWordAtStartRun = this.modelIterator.charOffset;
    };
    BoxGenerator.prototype.isLastParagraphInCell = function () {
        var paragraph = this.run.paragraph;
        var tableCell = paragraph.getTableCell();
        if (!tableCell)
            return false;
    ...
    BoxGenerator.prototype.makeAnchoredTextBoxRun = function () {
        ...
        if (textBoxWrap)
            this.manager.anchoredObjectsManager.textBoxContextSizeCalculators[anchorTextBox.anchoredObjectID] =
                new anchored_objects_manager_1.AnchoredTextBoxContextSizeCalculator(textBoxWrap, anchorTextBox.size, this.subDocument.documentModel.compatibilitySettings.getActualLayoutInTableCell(anchorTextBox.anchorInfo));
        this.offsetStartWordAtStartRun = this.modelIterator.charOffset;
    };
    ...
    BoxGenerator.prototype.currWordToBox = function () {
        ...
        if (this.modelIterator.charOffset > this.offsetStartWordAtStartRun) {
            ...
            do {
                ...
                var charProps = this.getActualCharacterProperties();
    ...
    BoxGenerator.prototype.getActualCharacterProperties = function () {
        var fieldsInfo = this.currWrapInfo.fieldsInfo;
        if (fieldsInfo) {
            var isParentFieldTOC = false;
            for (var i = 0, fieldInfo = void 0; fieldInfo = fieldsInfo[i]; i++) {
                if (!isParentFieldTOC) {
                    if (fieldInfo.field.isHyperlinkField()) {
                        if (fieldInfo.field.getHyperlinkInfo().visited) {
                            var result = this.run.getCharacterMergedProperties().clone();
                            result.textColor = color_model_info_1.ColorModelInfo.makeByColor(color_1.ColorUtils.fromHashString("#483D8B"));
                            return result;
                        }
                        else
                            break;
                ...
                else if (fieldInfo.field.isHyperlinkField() && this.run.characterStyle.styleName == character_style_1.CharacterStyle.hyperlinkStyleName) {
                    ...
                    var characterStyle = this.run.characterStyle;
                    var characterStyleProperties = characterStyle.maskedCharacterProperties.clone();
                    characterStyleProperties.setUseValue(enums_1.CharacterPropertiesMask.UseUnderlineColorIndex, false);
                    characterStyleProperties.setUseValue(enums_1.CharacterPropertiesMask.UseForeColorIndex, false);
