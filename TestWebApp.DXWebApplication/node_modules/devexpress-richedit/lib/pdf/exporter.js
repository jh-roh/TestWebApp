...
var PdfExporter = (function () {
    ...
    PdfExporter.prototype.exportDocument = function (layout, fonts, fontsCollector, callback, modifyPdfDocument, modifyPdfPage) {
        var _this = this;
        this.docProcessor.modelManager.modelManipulator.picture.loader.ensureAllPicturesPdfCompatible(3000, this.pdfSettings.convertImageToCompatibleFormat, function () {
            var doc = new _this.pdfSettings.pdfDocument({ autoFirstPage: false, font: 'Symbol' });
            var stream = new _this.pdfSettings.blobStream();
            ...
            for (var _i = 0, fonts_2 = fonts; _i < fonts_2.length; _i++) {
                var font = fonts_2[_i];
                var cacheElem = _this.pdfFontCache.getFont(font);
                if (cacheElem)
                    doc.registerFont(font.cacheKey, cacheElem.data, cacheElem.isFontCollection ? cacheElem.fontFamily : undefined);
            }
            var layoutFontsCollCache = _this.replaceUnloadedFonts(fontsCollector);
            var pageExporter = new page_exporter_1.PdfLayoutPageExporter(doc, layoutFontsCollCache, _this.measurer);
