...
exports.NumberingListFormPreviewHelper = exports.CustomListlevel = exports.DialogCustomNumberingListParameters = exports.DialogCustomNumberingListCommand = void 0;
...
var DialogCustomNumberingListCommand = (function (_super) {
    ...
    DialogCustomNumberingListCommand.prototype.applyToCustomListLevels = function (sourcelevels, targetlevels, abstractNumberingListIndex) {
        var modelManipulator = this.modelManipulator;
        var history = this.history;
        for (var i = 0, length_1 = targetlevels.length; i < length_1; i++) {
            var level = targetlevels[i];
            var initLevel = sourcelevels[i];
    ...
    DialogCustomNumberingListCommand.prototype.applyToListLevels = function (sourcelevels, targetlevels, abstractNumberingListIndex) {
        var numberingListManipulator = this.modelManipulator.numberingList;
        var activeSubDocument = this.selection.activeSubDocument;
        var model = activeSubDocument.documentModel;
        for (var i = 0, length_2 = targetlevels.length; i < length_2; i++) {
            var targetLevel = targetlevels[i];
            var sourceLevel = sourcelevels[i];
            var levelPropertiesManipulator = numberingListManipulator.listLevelProperties;
            ...
            var paragraphManipulator = numberingListManipulator.listLevelParagraphProperties;
            ...
            var listLevelCharacterProperties = numberingListManipulator.listLevelCharacterProperties;
            var fontInfo = targetLevel.getCharacterProperties().fontInfo.clone();
    ...
    DialogCustomNumberingListCommand.prototype.applyCustomNumberingListParameters = function (initParams, newParams) {
        var activeSubDocument = this.selection.activeSubDocument;
        var firstParagraph = activeSubDocument.getParagraphByPosition(this.selection.intervals[0].start);
        var isMultilevel = this.listType == numbering_list_1.NumberingType.MultiLevel;
        if (initParams.equals(newParams) || isMultilevel) {
            ...
            if (isMultilevel) {
                var abstractNumberingListIndex_1 = firstParagraph.getAbstractNumberingListIndex();
                this.applyToListLevels(newParams.levels, newParams.initAbstractNumberingList.levels, abstractNumberingListIndex_1);
            }
            return;
        }
...
var NumberingListFormPreviewHelper = (function () {
    ...
    NumberingListFormPreviewHelper.prototype.getNumberingListBoxText = function (levelIndex, majorIndex) {
        ...
        return (_a = this.richEdit.modelManager.model.simpleFormattersManager).formatString.apply(_a, tslib_1.__spreadArray([displayFormatString], items));
    };
